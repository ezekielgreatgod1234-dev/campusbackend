...
const MAIL_FROM = process.env.MAIL_FROM || "noreply@campusmart.app";
...
async function requireAdmin(req) {
  ...
  const adminEmail = String(decoded.email || "")
    .trim()
    .toLowerCase();
...
app.post("/admin/publish-ticker", async (req, res) => {
  try {
    const decoded = await requireAdmin(req);

    const { mode, email, message } = req.body || {};

    const cleanMode = String(mode || "all")
      .trim()
      .toLowerCase();
    ...
    if (cleanMode === "all") {
      ...
      console.log("GLOBAL TICKER PUBLISHED", {
        message: cleanMessage,
      ...
      return res.json({
        ...
        message: "Global ticker published successfully.",
      });
    }
    ...
    if (cleanMode === "single") {
       ...
       * Firebase Authentication is the source of truth.
       */
      const userRecord = await findFirebaseUserByEmail(normalizedEmail);

      if (userRecord.disabled) {
        return res.status(400).json({
          error: "This Firebase account is disabled.",
        });
      }
      ...
      await publishPrivateTicker({
        ...
        targetEmail: userRecord.email || normalizedEmail,
      ...
      console.log("PRIVATE TICKER PUBLISHED", {
        targetEmail: userRecord.email || normalizedEmail,
      ...
      return res.json({
        ...
        message: "Private ticker sent successfully.",
      });
    }

    return res.status(400).json({
      error: 'mode must be either "all" or "single"',
    });
  } catch (error) {
    console.error("Publish ticker error:", error);

    return res.status(error.status || 500).json({
      error: error.message || "Could not publish ticker",
    });
  }
});
...
app.post("/admin/clear-ticker", async (req, res) => {
  try {
    const decoded = await requireAdmin(req);
    ...
    if (cleanMode === "all") {
        ...
        .set(
          {
            ...
            clearedByEmail: decoded.email || null,
          },
          {
            merge: true,
          },
      ...
      console.log("GLOBAL TICKER CLEARED");
    ...
    if (cleanMode === "single") {
      ...
      console.log("PRIVATE TICKER CLEARED", {
        targetUid: userRecord.uid,
      ...
      return res.json({
        ...
        message: "Private ticker cleared.",
      });
    }
    ...
    console.error("Clear ticker error:", error);

    return res.status(error.status || 500).json({
      error: error.message || "Could not clear ticker",
    });
  }
});
...
app.get("/admin/lookup-user", async (req, res) => {
  try {
    ...
    return res.status(error.status || 500).json({
      error: error.message || "Could not find user",
    });
  }
});
...
app.post("/admin/private-ticker", async (req, res) => {
  try {
    const decoded = await requireAdmin(req);
    ...
    console.error("Private ticker error:", error);

    return res.status(error.status || 500).json({
      error: error.message || "Could not publish private ticker",
    });
  }
});
...
app.post("/admin/private-ticker/clear", async (req, res) => {
  try {
    const decoded = await requireAdmin(req);
    ...
    console.error("Clear private ticker error:", error);

    return res.status(error.status || 500).json({
      error: error.message || "Could not clear private ticker",
    });
  }
});
...
app.post("/admin/global-ticker", async (req, res) => {
  try {
    const decoded = await requireAdmin(req);
    ...
    return res.status(error.status || 500).json({
      error: error.message || "Could not publish global ticker",
    });
  }
});
...
app.post("/admin/global-ticker/clear", async (req, res) => {
  try {
    await requireAdmin(req);

    await db.collection("settings").doc("liveTicker").set(
      {
        active: false,
    ...
    console.error("Clear global ticker error:", error);

    return res.status(error.status || 500).json({
      error: error.message || "Could not clear global ticker",
    });
  }
});
...
app.post("/process-withdrawal", async (req, res) => {
  try {
    ...
    if (deduction && deduction.error) {
      return res.status(deduction.status).json({
...
app.post("/process-platform-withdrawal", async (req, res) => {
  try {
    ...
    const transferReference = `PFEE_${Date.now()}`;
...
const WELCOME_EMAIL_SUBJECT = "Welcome to CampusMart 2.0";

const WELCOME_SUPPORT_EMAIL = "campusmart1234@gmail.com";
...
function buildWelcomeEmailHtml(name) {
  ...
  return `
    ...
    >
      ...
      ${bullet("Pay Securely", "Enjoy secure payments powered by Paystack.")}
      ${bullet(
        "Track Your Orders",
...
app.post("/send-welcome-email", async (req, res) => {
  try {
    ...
    if (!userRecord.emailVerified && !isAdminAccount) {
      return res.status(403).json({
        ...
        error: "Welcome email is only sent after the email address is verified",
      });
    }
    ...
    try {
      ...
      console.warn("welcome email: could not read user doc:", err.message);
    }
    ...
    if (userData.welcomeEmailSent === true) {
      return res.json({
        ...
        message: "Welcome email was already sent to this account",
      });
    }
    ...
    const subject = (customTitle || WELCOME_EMAIL_SUBJECT).replace(
      /\{\{?name\}?\}/g,
      name,
    );
    ...
    const bodyText = customBody
      ? customBody.replace(/\{\{?name\}?\}/g, name)
    ...
    try {
      ...
      return res.json({
        ...
        message: "Could not send the welcome email right now",
      });
    }
    ...
    try {
      ...
      console.warn("welcome email: could not flag user doc:", err.message);
    }
...
app.post("/send-announcement-email", async (req, res) => {
  try {
    ...
    const { title, body, mode, email, showBanner = true } = req.body || {};
    ...
    if (mode === "single") {
       ...
       */
      const targetUser = await findFirebaseUserByEmail(normalizedEmail);
...
function firstExistingValue(object, keys, fallback = null) {
...
function normalizeProduct(docSnap, sellerData = {}) {
  const data = docSnap.data() || {};
  ...
  const image =
    firstExistingValue(
      data,
      ["image", "imageUrl", "imageURL", "photoURL", "thumbnail", "coverImage"],
      null,
    ) ||
    rawImages[0] ||
    null;

  const name = firstExistingValue(
    data,
  ...
  const price = numericValue(
    firstExistingValue(data, ["price", "amount", "sellingPrice"], null),
  );

  const sellerId = firstExistingValue(
    data,
    ["sellerId", "sellerUid", "sellerID", "userId", "ownerId"],
    null,
  );
  ...
  const sellerName =
    firstExistingValue(
      data,
      ["sellerName", "sellerDisplayName", "vendorName"],
      null,
    ) ||
    firstExistingValue(
      sellerData,
  ...
  const location =
    firstExistingValue(data, ["location", "campus", "sellerCampus"], null) ||
    firstExistingValue(sellerData, ["campus", "location", "address"], null);

  const category = firstExistingValue(
    data,
    ["category", "categoryName", "type"],
    null,
  );

  const description = firstExistingValue(
    data,
  ...
  const stock = numericValue(
    firstExistingValue(data, ["stock", "quantity", "availableQuantity"], null),
  );

  const status = firstExistingValue(data, ["status", "availability"], null);
  ...
  return {
    ...
    url: `/products/${docSnap.id}`,
  };
}

async function getUserProfileForAi(uid) {
  if (!uid) {
    return {};
  }
  ...
  try {
    ...
    console.error("CampusMart AI user profile lookup error:", error.message);

    return {};
  }
}

async function searchCampusMartProducts(args = {}) {
  const rawQuery = String(args.query || "")
    .trim()
    .toLowerCase();

  const minPrice = numericValue(args.minPrice);

  const maxPrice = numericValue(args.maxPrice);

  const category = String(args.category || "")
    .trim()
    .toLowerCase();

  const campus = String(args.campus || "")
    .trim()
    .toLowerCase();
  ...
  const meaningfulTokens = rawTokens.filter(
    (w) => !stopWords.has(w) && w.length >= 2 && !/^\d+(\.\d+)?$/.test(w),
  );

  const queryWords = expandTokens(
  ...
  const isBrowseRequest =
    ...
    (meaningfulTokens.length <= 1 &&
      ["all", "list", "browse", "catalog", "marketplace"].includes(
        meaningfulTokens[0] || "",
      ));
  ...
  try {
    snapshot = await db.collection("products").limit(400).get();
  } catch (error) {
    console.error("CampusMart product search error:", error);

    throw new Error("Could not search CampusMart products.");
  }

  const docs = snapshot.docs;

  console.log("CampusMart product search:", {
    rawQuery,
    meaningfulTokens,
  ...
  for (let i = 0; i < sellerIdArray.length; i += 10) {
    const batch = sellerIdArray.slice(i, i + 10);
    ...
    await Promise.all(
      batch.map(async (sellerId) => {
        try {
          ...
          sellerMap.set(
            sellerId,
            sellerSnap.exists ? sellerSnap.data() || {} : {},
          );
        } catch {
  ...
  for (const docSnap of docs) {
    ...
    const rawStatus = String(
      firstExistingValue(
        data,
        ["status", "availability", "productStatus"],
        "active",
      ),
    ).toLowerCase();
    ...
    const sellerData = sellerId ? sellerMap.get(String(sellerId)) || {} : {};

    const product = normalizeProduct(docSnap, sellerData);

    /*
     * Normalize price strings like "₦120,000" if needed.
     */
    if (product.price === null) {
      const rawPrice = firstExistingValue(
        data,
      ...
      if (typeof rawPrice === "string") {
        ...
        product.price = numericValue(cleaned);
      }
    }
    ...
    if (
      campus &&
      !String(product.location || product.campus || "")
        .toLowerCase()
        .includes(campus)
    ) {
      continue;
    ...
    const brandText = data.brand ? String(data.brand).toLowerCase() : "";
    ...
    if (isBrowseRequest) {
      ...
      if (phrase && nameText.includes(phrase)) {
        score += 25;
        matchedOnProduct = true;
      }

      if (phrase && categoryText.includes(phrase)) {
        score += 22;
        matchedOnProduct = true;
      }
    } else {
     ...
     */
    if (!isBrowseRequest && queryWords.length && !matchedOnProduct) {
      continue;
    }
  ...
  console.log("CampusMart product search results:", results.length);

  /*
   * No soft-fallback of unrelated products.
   * If the user asked for food, only food (or nothing) is returned.
   */
  return results.slice(0, 12).map(({ _score, ...product }) => product);
}

async function searchCampusMartGigs(args = {}) {
  const rawQuery = String(args.query || "")
    .trim()
    .toLowerCase();
  ...
  const minBudget = numericValue(args.minBudget);

  const maxBudget = numericValue(args.maxBudget);
  ...
  const topicTokens = meaningfulTokens.filter((w) => !browseWords.has(w));

  /*
   * "show me gigs" / "find gigs" / empty topic → browse all open gigs.
   */
  const isBrowseRequest = topicTokens.length === 0;

  const queryWords = topicTokens.length ? topicTokens : [];
  ...
  try {
     ...
     */
    snapshot = await db.collection("gigs").limit(300).get();

    if (snapshot.empty) {
      const alt = await db.collection("gig").limit(300).get();
    ...
    console.error("CampusMart gig search error:", error);

    throw new Error("Could not search CampusMart gigs.");
  }

  console.log("CampusMart gig search:", {
    rawQuery,
    topicTokens,
    isBrowseRequest,
  ...
  for (const docSnap of snapshot.docs) {
    ...
    const title = firstExistingValue(
      data,
      ["title", "name", "gigTitle", "gigName", "jobTitle"],
      "Campus Gig",
    );

    const description = firstExistingValue(
      data,
      ["description", "details", "gigDescription", "about", "summary"],
      "",
    );

    const gigCategory = firstExistingValue(
      data,
      ["category", "type", "gigCategory", "jobType", "skill", "skills"],
      "",
    );

    const location = firstExistingValue(
      data,
      ["location", "campus", "school", "university", "address"],
      "",
    );
    ...
    const budget = numericValue(
      firstExistingValue(
        data,
        ["budget", "price", "amount", "pay", "payment", "rate"],
        null,
      ),
    );
    ...
    if (category && !String(gigCategory).toLowerCase().includes(category)) {
      continue;
    }

    if (campus && !String(location).toLowerCase().includes(campus)) {
      continue;
    }

    const titleText = String(title).toLowerCase();

    const categoryText = String(gigCategory).toLowerCase();
  ...
  console.log("CampusMart gig search results:", results.length);

  return results.slice(0, 12).map(({ _score, ...gig }) => gig);
}

async function getMyCampusMartOrders(uid) {
  if (!uid) {
  ...
  try {
    ...
    console.error("CampusMart order lookup error:", error);

    throw new Error("Could not retrieve your CampusMart orders.");
  }
  ...
  return snapshot.docs.map((docSnap) => {
    ...
    return {
      ...
      total: numericValue(data.total, 0),
      ...
      items: items.slice(0, 20).map((item) => ({
        ...
        price: numericValue(item?.price, null),
      })),
    };
  });
}
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Short gig keywords, e.g. tutoring, design, repair.",
          },
          category: {
            type: "string",
...
function serializeAiMessagesForStore(messages) {
  ...
  return messages
    .filter(
      (m) =>
        m && (m.role === "user" || m.role === "ai" || m.role === "assistant"),
    )
    .map((m, index) => {
      const role = m.role === "assistant" || m.role === "ai" ? "ai" : "user";

      return {
        id: String(m.id || `${role}-${index}-${Date.now()}`).slice(0, 80),
        ...
        products:
          role === "ai" && Array.isArray(m.products)
            ? m.products.slice(0, 12)
            : [],

        gigs: role === "ai" && Array.isArray(m.gigs) ? m.gigs.slice(0, 12) : [],
...
async function loadAiConversation(uid) {
  ...
  try {
    const snap = await db.collection("aiConversations").doc(uid).get();
    ...
    return {
      messages: serializeAiMessagesForStore(data.messages || []),
...
async function saveAiConversation({ uid, firstName, messages }) {
  if (!uid) {
    return;
  }

  const clean = serializeAiMessagesForStore(messages);
...
async function deleteAiConversation(uid) {
  ...
  await db.collection("aiConversations").doc(uid).delete();
}
...
async function deleteAiMessageFromConversation(uid, messageId) {
  ...
  const ref = db.collection("aiConversations").doc(uid);
  ...
  try {
    ...
    if (!workingModel) {
      return res.status(502).json({
        ...
        baseURL: GEMINI_BASE_URL,
        keyPresent: true,
        keyPreview: String(GEMINI_API_KEY).slice(0, 6) + "...",
        tried,
        error: lastError || "No Gemini model responded.",
        help: "Set GEMINI_AI_MODEL=gemini-3.6-flash (Google recommendation for new API keys).",
      });
    }
    ...
    return res.json({
      ...
      keyPreview: String(GEMINI_API_KEY).slice(0, 6) + "...",
      tried,
      message: "CampusMart AI can reach Gemini successfully.",
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (!uid) {
      return res.status(401).json({
        success: false,
        error: "Please log in to use CampusMart AI.",
      });
    }

    const stored = await loadAiConversation(uid);
    ...
    if (stored.updatedAt && typeof stored.updatedAt.toDate === "function") {
      updatedMs = stored.updatedAt.toDate().getTime();
    } else if (stored.updatedAt) {
      updatedMs = new Date(stored.updatedAt).getTime();
    }

    const hoursSince = updatedMs
      ? (Date.now() - updatedMs) / (1000 * 60 * 60)
      : null;

    const returningAfter24h = hoursSince !== null && hoursSince >= 24;
    ...
    return res.status(error.status || 500).json({
      success: false,
      error: error.message || "Could not load chat history.",
    });
  }
});
...
app.delete("/ai/history", async (req, res) => {
  try {
    const decoded = await verifyFirebaseUser(req);
    ...
    return res.json({
      success: true,
      message: "CampusMart AI chat history was deleted.",
    });
  } catch (error) {
    ...
    return res.status(error.status || 500).json({
      success: false,
      error: error.message || "Could not delete chat history.",
    });
  }
});
...
app.delete("/ai/history/messages/:messageId", async (req, res) => {
  try {
    const decoded = await verifyFirebaseUser(req);
    ...
    const result = await deleteAiMessageFromConversation(uid, messageId);
    ...
    console.error("AI message delete error:", error);

    return res.status(error.status || 500).json({
      success: false,
      error: error.message || "Could not delete that message.",
    });
  }
});
  ...
  try {
    ...
    if (isAiQuotaOnCooldown()) {
      return res.status(429).json({
        ...
        retryAfterSeconds: getAiQuotaCooldownSecondsLeft(),
      });
    }
     ...
     */
    const userProfile = await getUserProfileForAi(uid);
    ...
    const role = firstExistingValue(
      userProfile,
      ["role"],
      decoded.role || "customer",
    );

    const campus = firstExistingValue(
      userProfile,
      ["campus", "school", "university", "location"],
      null,
    );

    const email = decoded.email || userProfile.email || "";
     ...
     */
    const previousMessages = cleanAiMessages(req.body?.messages);
    ...
    for (const message of conversation) {
      const previous =
        normalizedConversation[normalizedConversation.length - 1];
      ...
      normalizedConversation.push(message);
    }

    /*
     * Trusted system instructions (kept short for speed).
     */
    const instructions = buildCampusMartAiInstructions({
      uid,
      fullName,
      firstName,
      email,
    ...
    console.log("CampusMart AI using model:", activeModel);
      ...
      for (const toolCall of toolCalls) {
        ...
        try {
          if (toolName === "search_products") {
            const products = await searchCampusMartProducts(args);
            ...
            toolResult = {
              ...
              products: products.map((p) => ({
                ...
                location: p.location || p.campus || null,
                category: p.category || null,
              })),
            };
          } else if (toolName === "search_gigs") {
            const gigs = await searchCampusMartGigs(args);
            ...
            toolResult = {
              ...
              gigs: gigs.map((g) => ({
                ...
                description: String(g.description || "").slice(0, 100),
              })),
            };
          } else if (toolName === "get_my_orders") {
            const orders = await getMyCampusMartOrders(uid);
          ...
          console.error(`CampusMart AI tool error (${toolName}):`, toolError);
    ...
    if (!reply) {
      return res.status(502).json({
        ...
        error: "CampusMart AI could not generate a reply. Please try again.",
      });
    }
    ...
    try {
      const historyForStore = [
...

        {
          ...
          products: Array.isArray(finalProducts) ? finalProducts : [],

          gigs: Array.isArray(finalGigs) ? finalGigs : [],
      ...
      await saveAiConversation({
        ...
        messages: historyForStore,
      });
    } catch (saveErr) {
      console.error("AI save after chat failed:", saveErr.message);
    }
    ...
    if (clientDisconnected || isAbort) {
      console.log("CampusMart AI reply stopped by client.");

      return;
    }

    console.error("CampusMart AI endpoint error:", error);
    ...
    console.error("CampusMart AI raw error:", {
      status,
      message,
...
app.post("/notify-new-message", async (req, res) => {
  try {
    ...
    const senderName =
      String(req.body?.senderName || "Someone").trim() || "Someone";
    ...
    try {
      ...
      if (pSnap.exists) {
        ...
        const boosted =
          ...
          p.recommended === true ||
          (p.boostedUntil &&
            new Date(p.boostedUntil.toDate?.() || p.boostedUntil) > new Date());
        isFeatured = !!boosted;
...
app.post("/send-test-push", async (req, res) => {
  try {
    ...
    const body =
      String(
        req.body?.body || "Push notifications are working on this device.",
...
app.listen(PORT, () => {
  ...
  console.log(`CampusMart AI provider: Google Gemini`);

  console.log(`CampusMart AI model: ${CAMPUSMART_AI_MODEL}`);

  console.log(`CampusMart AI base URL: ${GEMINI_BASE_URL}`);
});
